...
(defun init-gl (screen-width screen-height window gl-context)
  (with-gensyms (w h)
    `(let ((,w ,screen-width) (,h ,screen-height))
       (sdl2:gl-make-current ,window ,gl-context)
       (gl:matrix-mode :projection)
       (gl:load-identity)
       (gl:clear-color ,@(get-color :white))
       (gl:viewport 0 0 ,w ,h)
...
(defun game (quit-key move-keys fps)
  (with-gensyms (head body direction fruit win gl-context previous-x previous-y)
    `(let ((,head (make-square '(,(/ *screen-width* 2) ,(/ *screen-height* 2))))
	 ...
	 (sdl2:with-window (,win :title "Snake"  :w ,*screen-width* :h ,*screen-height* :flags '(:shown :opengl))
	   (sdl2:with-gl-context (,gl-context ,win)
	     ,(init-gl *screen-width* *screen-height* win gl-context)
	     (sdl2:with-event-loop (:method :poll)
	       ,(quit-game quit-key)
	       ,(set-direction direction move-keys)
	       (:idle ()
		      ,(move-snake head direction previous-x previous-y)
		      (body-intersect-p ,head ,body)
    ...
    (labels ((rec (commands)
		 ...
		 (case (car c)
		   ...
		   (move-keys
		    ...
		    (rec (cdr commands)))))))
      (rec commands)
      (mapcar #'(lambda (k h) (gethash k h)) '(quit move fps) (list args args args)))))
